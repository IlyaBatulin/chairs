...
document.addEventListener('DOMContentLoaded', function() {
    preloadImages([
        "РАЗДЕЛЫ/КРЕС С МАССАЖ/1.jpg",
        "РАЗДЕЛЫ/ДЕТ КРЕС/2.jpg",
        "РАЗДЕЛЫ/СТУЛЬЯ/3.jpg",
        "РАЗДЕЛЫ/КРЕС С МАССАЖ/классические/11.jpg",
        "РАЗДЕЛЫ/КРЕС С МАССАЖ/спортивные/10.jpg",
        "РАЗДЕЛЫ/ДЕТ КРЕС/1detchair.jpg",
        "РАЗДЕЛЫ/ДЕТ КРЕС/2detchair.jpg",
        "РАЗДЕЛЫ/СТУЛЬЯ/модели/1.jpg",
        "РАЗДЕЛЫ/СТУЛЬЯ/модели/2.jpg",
        "РАЗДЕЛЫ/СТУЛЬЯ/модели/3.jpg",
        "РАЗДЕЛЫ/СТУЛЬЯ/модели/4.jpg",
        "РАЗДЕЛЫ/СТУЛЬЯ/модели/5.jpg",
        "РАЗДЕЛЫ/СТУЛЬЯ/модели/6.jpg",
        "РАЗДЕЛЫ/СТУЛЬЯ/модели/7.jpg",
        "РАЗДЕЛЫ/СТУЛЬЯ/модели/8.jpg"
    ]);
    
    // Функция для предварительной загрузки изображений
    function preloadImages(sources) {
        console.log("Предварительная загрузка изображений...");
        for (let i = 0; i < sources.length; i++) {
            const img = new Image();
            img.onload = function() {
                console.log("Изображение загружено: " + sources[i]);
            };
            img.onerror = function() {
                console.error("Ошибка загрузки изображения: " + sources[i]);
                retryLoadImage(sources[i]);
            };
            img.src = sources[i];
        }
    }
    ...
    function retryLoadImage(src) {
        // Пробуем загрузить с другим путем
        const alternativeSrc = src.replace("РАЗДЕЛЫ/", "");
        console.log("Пробуем альтернативный путь: " + alternativeSrc);
        
        const img = new Image();
        img.onload = function() {
            console.log("Изображение загружено через альтернативный путь: " + alternativeSrc);
            
            // Заменяем все src на странице
            document.querySelectorAll('img[src="' + src + '"]').forEach(function(imgElement) {
                imgElement.src = alternativeSrc;
                console.log("Изменен src элемента на: " + alternativeSrc);
            });
        };
        img.onerror = function() {
            console.error("Ошибка загрузки изображения через альтернативный путь: " + alternativeSrc);
        };
