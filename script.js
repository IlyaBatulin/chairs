...
// Улучшенная функция для работы с корзиной
function updateCart() {
    ...
    document.querySelector('.cart-total').textContent = `Итого: ${total} ₽`;
    updateCartButton();
...
function updateQuantity(index, change) {
    if (cart[index]) {
        ...
        updateCart();
        showCartNotification(`Количество товара обновлено`);
    }
}

// Улучшенная функция сохранения корзины
...
// Улучшенное уведомление для мобильных устройств
function showCartNotification(message) {
    const notification = document.createElement('div');
    notification.className = 'cart-notification';
    notification.textContent = message;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Добавляем обработчик для закрытия корзины по клику вне её области
    document.addEventListener('click', (e) => {
